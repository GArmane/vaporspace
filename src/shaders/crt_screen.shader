...
// Curvature
uniform float screen_curvature = 1.0;
// Color bleeding
...
vec2 distort(vec2 p) {
	...
	float radius = pow(length(p), screen_curvature);
...
void fragment() {
	...
	// Controls if a pixel should be distorted
	// Greater the number, more pixels are
	// distorted in the edges
	float d = length(xy);
	if(d < 1.5) {
		xy = distort(xy);
	}
